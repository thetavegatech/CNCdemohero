...
const TPMSForm = () => {
  ...
  const fetchTpmsData = async () => {
    try {
      // const machineId = '101'; // Example machineId
      // const response = await axios.get(`http://localhost:5001/api/tpmsdata/machine/${machineId}`);
      const response = await axios.get(`http://localhost:5001/api/tpmsdata/machine`);
  ...
  return (
    <div className="container3">
      <h2 className="mb-4">TPMS Form</h2>
          ...
          {tpmsData.map(item => (
            <tr key={item._id}>
              ...
              <td>
                ...
                <button className="btn btn-danger btn-sm " onClick={() => deleteTpms(item._id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
